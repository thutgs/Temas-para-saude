...
```{r}
ggplot(banco) +
  aes(x = Ano, y = log(agravo_indicador*10000)) +
  geom_boxplot(fill = c("dark blue"), width = 0.5) +
  stat_summary(
    fun = "mean", geom = "point", shape = 23, size = 3, fill = "white"
  ) +
  labs(x = "Ano", y = "Transformação logarítimica Indicador de \nagravos neglicenciados por 10.000 habitantes") +
  theme_light()
```
...
banco %>%
  na.omit() %>% 
  group_by(Ano) %>% 
  summarise(média = mean(agravo_indicador*10000), 
            mediana = median(agravo_indicador*10000),
            `desvio padrão` = sd(agravo_indicador*10000))
```
