...
options(scipen = 999)
library(pacman)
p_load(readxl, openxlsx, tidyverse, dplyr, reshape2, kableExtra, moments)
```
...
```{r}
ggplot(banco) +
  aes(x = Ano, y = agravo_indicador*100000) +
  geom_boxplot(fill = c("dark blue"), width = 0.5) +
  stat_summary(
    fun = "mean", geom = "point", shape = 23, size = 3, fill = "white"
  )+
  labs(x = "Ano", y = "Indicador de agravos neglicenciados\n por 100.000 habitantes") +
  theme_light()
```
...
banco %>%
  na.omit() %>% 
  group_by(Ano) %>% 
  summarise(média = mean(agravo_indicador*100000),
            `desvio padrão` = sd(agravo_indicador*100000),
            máximo = max(agravo_indicador*100000),
            `1_quartil` = quantile(agravo_indicador*100000, .25),
            mediana = median(agravo_indicador*100000),
            `3_quartil` = quantile(agravo_indicador*100000, .75),
            assimetria = skewness(agravo_indicador*100000),
            curtose = kurtosis(agravo_indicador*100000)) 
```

```{r}
banco %>%
  na.omit(agravo_indicador) %>% 
  group_by(Uf) %>% 
  summarise(`Média de agravo` = mean(agravo_indicador*100000)) %>% 
  arrange(desc(`Média de agravo`)) %>% 
  head(10) %>% 
  kable(caption = "10 UFs com maior média de agravo por 100.000 habitantes")
```
